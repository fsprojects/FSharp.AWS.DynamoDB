...
type PrimaryKeyStructure with
    ...
    static member ExtractKey(keyStructure: PrimaryKeyStructure, attributeValues: Dictionary<string, AttributeValue>) =
        let inline getValue (rp: PropertyMetadata) =
            let notFound () =
                raise <| KeyNotFoundException(sprintf "attribute %A not found." rp.Name)
...
type KeyAttributeSchema with
    ...
    static member Create(name: string, pickler: Pickler, allowNull: bool) =
        if
            pickler.PicklerType <> PicklerType.Value
            && pickler.PicklerType <> PicklerType.Wrapper
        then
            invalidArg name
            <| sprintf "Unsupported type '%O' for DynamoDB Key attribute." pickler.Type
    ...
    static member Create(prop: PropertyMetadata, allowNull: bool) =
        KeyAttributeSchema.Create(prop.Name, prop.Pickler, allowNull)

type TableKeySchema with

    static member OfKeyStructure(ks: PrimaryKeyStructure) : TableKeySchema =
        let inline mkKeySchema (name: string) (pickler: Pickler) =
            KeyAttributeSchema.Create(name, pickler, false)

        let inline mkPropSchema (rp: PropertyMetadata) = KeyAttributeSchema.Create(rp, false)

        let inline mkTableKeySchema h r =
            { HashKey = h
              RangeKey = r
              Type = PrimaryKey }
...
type RecordTableInfo with
        ...
        let extractKeySchema (kst: KeySchemaType) (attributes: seq<PropertyMetadata * KeyType * KeySchemaType>) =
            ...
            | LocalSecondaryIndex _, [| (KeyType.Range, rk) |] ->
                ...
                | Some(_, ks) when Option.isNone ks.RangeKey ->
                    "LocalSecondaryIndex tables must specify a primary RangeKey"
                    |> invalidArg (string typeof<'T>)

                | Some(_, ks) ->
                    { ks with
                        RangeKey = Some(mkKAS rk)
                        Type = kst }
            ...
            | GlobalSecondaryIndex _, [| (KeyType.Hash, hk) |] ->
                { HashKey = mkKAS hk
                  RangeKey = None
                  Type = kst }
            ...
            | GlobalSecondaryIndex id, _ ->
                sprintf
                    "Invalid combination of GlobalSecondaryHashKey and GlobalSecondaryRangeKey attributes for index name '%s'."
                    id
                |> invalidArg (string typeof<'T>)
        ...
        | Some(pkStruct, pkSchema) ->
            ...
            let propSchema =
                ...
                |> Seq.map (fun (name, kss) ->
                    let schemata =
                        kss |> Seq.map (fun (_, isHashKey, ks) -> ks, isHashKey) |> Seq.toArray
    ...
    static member ExtractIndexKey
        ...
        let inline getValue (ks: KeyAttributeSchema) =
            let notFound () =
                raise
                <| KeyNotFoundException(sprintf "attribute %A not found." ks.AttributeName)
        ...
        | { HashKey = hk
            RangeKey = Some rk
            Type = LocalSecondaryIndex _ }
        | { HashKey = hk
            RangeKey = Some rk
            Type = GlobalSecondaryIndex _ } ->
            let hash = getValue hk
            let range = getValue rk
    ...
    static member IndexKeyToAttributeValues
        (
            indexKeySchema: TableKeySchema,
            recordInfo: RecordTableInfo,
            key: IndexKey
        ...
        let dict =
            PrimaryKeyStructure.ToAttributeValues(recordInfo.PrimaryKeyStructure, key.PrimaryKey)
...
type TableKeySchemata with
    ...
    static member OfTableDescription(td: TableDescription) : TableKeySchemata =
        let mkKeySchema (kse: KeySchemaElement) =
            let ad =
                td.AttributeDefinitions
                |> Seq.find (fun ad -> ad.AttributeName = kse.AttributeName)

            { AttributeName = kse.AttributeName
              KeyType = ad.AttributeType }
        ...
        let mkGlobalSecondaryIndex (gsid: GlobalSecondaryIndexDescription) : TableKeySchema =
            ...
            { HashKey = gsid.KeySchema |> Seq.find (fun ks -> ks.KeyType = KeyType.HASH) |> mkKeySchema
              RangeKey =
                gsid.KeySchema
                |> Seq.tryFind (fun ks -> ks.KeyType = KeyType.RANGE)
                |> Option.map mkKeySchema
        ...
        let mkLocalSecondaryIndex (lsid: LocalSecondaryIndexDescription) : TableKeySchema =
            ...
            { HashKey = primaryKey.HashKey
              RangeKey =
                lsid.KeySchema
                |> Seq.find (fun ks -> ks.KeyType = KeyType.RANGE)
                |> mkKeySchema
                |> Some
    ...
    member schema.ApplyToCreateTableRequest(ctr: CreateTableRequest) =
        ...
        for tks in schema.Schemata do
            ...
            | PrimaryKey ->
                ...
                tks.RangeKey
                |> Option.iter (fun rk -> ctr.KeySchema.Add <| mkKSE rk.AttributeName KeyType.RANGE)
            ...
            | GlobalSecondaryIndex name ->
                ...
                tks.RangeKey
                |> Option.iter (fun rk -> gsi.KeySchema.Add <| mkKSE rk.AttributeName KeyType.RANGE)
            ...
            | LocalSecondaryIndex name ->
                ...
                tks.RangeKey
                |> Option.iter (fun rk -> lsi.KeySchema.Add <| mkKSE rk.AttributeName KeyType.RANGE)
