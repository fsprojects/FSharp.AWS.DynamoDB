...
type AttributeId =
    ...
    member id.Append nf =
        { id with
            NestedAttributes = id.NestedAttributes @ [ nf ] }
    ...
    member id.Apply(inputs: obj[]) =
        ...
        { id with
            NestedAttributes = id.NestedAttributes |> List.map applyField }
    ...
    static member TryExtract ((|PVar|_|): Expr -> int option) (record: Var) (info: RecordTableInfo) (e: Expr) =
        ...
        and mkAttrPath acc (ctx: PropertyMetadata[] option) rest =
            ...
            | Choice3Of3(et, ie) :: tail, None ->
                ...
                let inline mkAttrPath indexV =
                    mkAttrPath (Item(indexV, pickler, acc)) ctx tail
...
type private AttributeNode =
    { Value: string
      Children: ResizeArray<AttributeNode> }
