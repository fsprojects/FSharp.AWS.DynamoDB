...
let extractKeyCondition (qExpr: QueryExpr) =
    ...
    let rec isKeyCond qExpr =
        ...
        | BeginsWith(attr, _) when attr.IsRangeKey -> trySet rangeKeyRef attr
        | BeginsWith(attr, _) when attr.IsHashKey -> false
        | BeginsWith(attr, _) when attr.IsRangeKey -> trySet rangeKeyRef attr

        | BeginsWith _ -> false
        | Between(Attribute attr, (Value _ | Param _), (Value _ | Param _)) when attr.IsRangeKey ->
            trySet rangeKeyRef attr

        | Between _ -> false
        | Compare(cmp, Attribute attr, (Value _ | Param _))
        | Compare(cmp, (Value _ | Param _), Attribute attr) ->
            if attr.IsHashKey && cmp = EQ then trySet hashKeyRef attr
            elif attr.IsRangeKey then trySet rangeKeyRef attr
            else false
    ...
    if isKeyCond qExpr then
        ...
        | Some hk ->
            ...
            | Some rk ->
                Seq.joinBy
                    (fun (ks, kt) (ks', kt') -> kt = KeyType.Hash && kt' = KeyType.Range && ks = ks')
                    hk.KeySchemata
                    rk.KeySchemata
...
let ensureNotTautological query =
    match query with
    | False
    | True -> invalidArg "expr" "supplied query is tautological."
    | _ -> ()
...
let extractQueryExpr (recordInfo: RecordTableInfo) (expr: Expr) : ConditionalExpression =
    ...
    let invalidQuery () =
        invalidArg "expr" <| sprintf "Supplied expression is not a valid conditional."
    ...
    | Lambda(r, body) when r.Type = recordInfo.Type ->
        ...
        let rec extractOperand (pickler: Pickler option) (expr: Expr) =
            ...
            | _ when expr.IsClosed ->
                let pickler =
                    match pickler with
                    | Some c -> c
                    | None -> Pickler.resolveUntyped expr.Type
            ...
            | SpecificProperty <@ fun (s: string) -> s.Length @> (Some(AttributeGet attr), _, []) -> SizeOf attr.Id

            | SpecificProperty <@ fun (l: _ list) -> l.Length @> (Some(AttributeGet attr), _, []) -> SizeOf attr.Id

            | SpecificCall2 <@ List.length @> (None, _, _, [ AttributeGet attr ]) -> SizeOf attr.Id

            | SpecificProperty <@ fun (s: Set<_>) -> s.Count @> (Some(AttributeGet attr), _, []) -> SizeOf attr.Id

            | SpecificCall2 <@ Set.count @> (None, _, _, [ AttributeGet attr ]) -> SizeOf attr.Id

            | SpecificProperty <@ fun (m: Map<_, _>) -> m.Count @> (Some(AttributeGet attr), _, []) -> SizeOf attr.Id

            | SpecificProperty <@ fun (a: _[]) -> a.Length @> (Some(AttributeGet attr), _, []) -> SizeOf attr.Id

            | SpecificCall2 <@ Array.length @> (None, _, _, [ AttributeGet attr ]) -> SizeOf attr.Id
        ...
        let rec extractQuery (expr: Expr) =
            ...
            | SpecificCall <@ not @> (None, _, [ body ]) -> QueryExpr.ENot(extractQuery body)

            | AndAlso(left, right) -> QueryExpr.EAnd (extractQuery left) (extractQuery right)

            | OrElse(left, right) -> QueryExpr.EOr (extractQuery left) (extractQuery right)
            ...
            | SpecificCall2 <@ fun (x: Map<_, _>) y -> x.ContainsKey y @> (Some(AttributeGet attr), _, _, [ key ]) when
                key.IsClosed
                ->
                let nf = extractNestedField key
                Attribute_Exists(attr.Id.Append nf)
            ...
            | SpecificCall2 <@ EXISTS @> (None, _, _, [ AttributeGet attr ]) -> Attribute_Exists attr.Id

            | SpecificCall2 <@ NOT_EXISTS @> (None, _, _, [ AttributeGet attr ]) -> Attribute_Not_Exists attr.Id

            | SpecificCall2 <@ Array.isEmpty @> (None, _, _, [ AttributeGet attr ]) -> Attribute_Not_Exists attr.Id

            | SpecificCall2 <@ List.isEmpty @> (None, _, _, [ AttributeGet attr ]) -> Attribute_Not_Exists attr.Id

            | SpecificCall2 <@ Option.isSome @> (None, _, _, [ AttributeGet attr ]) -> Attribute_Exists attr.Id

            | SpecificCall2 <@ Option.isNone @> (None, _, _, [ AttributeGet attr ]) -> Attribute_Not_Exists attr.Id
...
type ConditionalExpression with
    ...
    member cond.Apply([<ParamArray>] parameters: obj[]) = applyParams cond parameters

    member cond.Write(writer: AttributeWriter) = writeConditionExpression writer cond
