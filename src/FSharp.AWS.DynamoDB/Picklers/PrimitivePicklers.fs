...
type BoolPickler() =
    ...
    override _.UnPickle a =
        if a.IsBOOLSet then a.BOOL else invalidCast a
...
type CharPickler() =
    ...
    override _.UnPickle a =
        if not <| isNull a.S then Char.Parse(a.S) else invalidCast a
...
let inline mkNumericalPickler< ^N
    ...
    { new NumRepresentablePickler< ^N >() with
        ...
        member _.Pickle num =
            AttributeValue(N = toString num) |> Some

        member _.UnPickle a =
            if not <| isNull a.N then parseNum a.N else invalidCast a
...
type DoublePickler() =
    ...
    override _.UnPickle a =
        if not <| isNull a.N then parse a.N else invalidCast a
...
type GuidPickler() =
    ...
    override _.UnPickle a =
        if not <| isNull a.S then Guid.Parse a.S else invalidCast a
...
type DateTimeOffsetPickler() =
    ...
    override _.UnPickle a =
        if not <| isNull a.S then parse a.S else invalidCast a
...
type TimeSpanPickler() =
    ...
    override _.Pickle t =
        AttributeValue(N = string t.Ticks) |> Some
...
type EnumerationPickler<'E, 'U when 'E: enum<'U> and 'E: struct and 'E :> ValueType and 'E: (new: unit -> 'E)>() =
    ...
    override _.Pickle e =
        AttributeValue(S = e.ToString()) |> Some
...
type OptionPickler<'T>(tp: Pickler<'T>) =
    ...
    override _.UnPickle a =
        if a.NULL then None else Some(tp.UnPickle a)
...
type StringRepresentationPickler<'T>(ep: StringRepresentablePickler<'T>) =
    ...
    override _.Pickle t =
        AttributeValue(S = ep.UnParse t) |> Some

    override _.UnPickle a =
        if notNull a.S then ep.Parse a.S else invalidCast a
