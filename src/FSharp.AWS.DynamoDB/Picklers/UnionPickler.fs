...
type UnionPickler<'U>(resolver: IPicklerResolver) =
    ...
    let mkUCD uci =
        ...
        let props =
            uci.GetFields() |> Array.mapi (PropertyMetadata.FromPropertyInfo resolver)
    ...
    member __.ToUnion(ro: RestObject) : 'U =
        let notFound name =
            raise <| new KeyNotFoundException(sprintf "attribute %A not found." name)
        ...
        | None ->
            let msg =
                sprintf "union case name %A does not correspond to type '%O'." tag typeof<'U>
    ...
    override __.DefaultValue =
        invalidOp <| sprintf "default values not supported for unions."
    ...
    override __.UnPickle a =
        if a.IsMSet then __.ToUnion a.M else invalidCast a
