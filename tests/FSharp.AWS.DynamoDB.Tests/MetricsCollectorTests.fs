...
let (|TotalCu|): ConsumedCapacity list -> float =
    Seq.sumBy (fun c -> c.CapacityUnits)
...
type Tests(fixture: TableFixture) =
    ...
    let (|ExpectedTableName|_|) name =
        if name = fixture.TableName then Some() else None
    ...
    let ``Collect Metrics on Transactional PutItem`` () =
        async {
            ...
            let requests =
                [ TransactWrite.Put(item, Some(compile <@ fun t -> NOT_EXISTS t.RangeKey @>)) ]
    ...
    let ``No Metrics on Canceled PutItem`` () =
        async {
            ...
            let requests =
                [ TransactWrite.Put(item, Some(compile <@ fun t -> EXISTS t.RangeKey @>)) ]
...
type BulkMutationTests(fixture: TableFixture) =
    ...
    let ``Collect Metrics on BatchPutItem`` () =
        let _results =
            sut.BatchPutItems(items |> Seq.map (fun i -> { i with LocalAttribute = 1000 }))
    ...
    let ``Collect Metrics on BatchDeleteItem`` () =
        let _keys =
            sut.BatchDeleteItems(items |> Seq.map (fun i -> TableKey.Combined(i.HashKey, i.RangeKey)))
...
type ``Bulk Read Operations``(fixture: ManyReadOnlyItemsFixture) =
    ...
    let ``Collect Metrics on BatchGetItem`` () =
        let items =
            sut.BatchGetItems(seq { for i in 0..99 -> TableKey.Combined(fixture.HashKey, i) })
